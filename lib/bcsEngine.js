/* ─────────────────────────────────────────────────────────────────
   ...
   Changes:
   1. Added "Severely_Underweight" as a valid category that can be
      returned, so downstream engines (calorieEngine, dietEngine)
      that receive this category won't silently fail.
      The JSON BCS_Automatic_Detection_Logic thresholds are used.
      If JSON doesn't define Severely_Underweight, we detect it
      here as deviation <= -20%.

   2. Fixed boundary condition in age matching:
      Old: ageMonths >= minMonths && ageMonths < maxMonths
      The last stage (maxMonths = null) needs ageMonths >= minMonths only.
      That was already handled — confirmed correct. No change needed.

   3. Added fallback: if no BCS_Automatic_Detection_Logic in JSON,
      use hardcoded thresholds that match the JSON spec.
─────────────────────────────────────────────────────────────────── */
...
export function calculateBCS({
  ...
  for (const stage of stages) {
    ...
    if (stage.min_age_weeks !== undefined) {
      ...
      maxMonths = (stage.max_age_weeks != null)
        ? stage.max_age_weeks / 4.345: null;
      ...
      maxMonths = (stage.max_age_years != null)
        ? stage.max_age_years * 12 : null;
    }
  ...
  /* ── Category detection ─────────────────────────────────────────
     Try JSON BCS_Automatic_Detection_Logic first.
     Fallback to hardcoded thresholds matching JSON spec
     (which aligns with the old engine.js inline BCS logic).
     FIX: Added Severely_Underweight as an explicit category.
  ── */
  ...
  /* ── Hardcoded fallback (mirrors JSON spec) ── */
  if (!matchedCategory) {
  ...
  let estimatedBCS  = 5; // default neutral
  ...
  if (estimateMap?.[matchedCategory]) {
    ...
    // Fallback BCS estimation from category
    const fallbackBCS = {
