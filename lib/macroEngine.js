/* ─────────────────────────────────────────────────────────────────
   macroEngine.js — FIXED

   Bugs fixed:
   1. Fiber ratio was silently excluded from totalRatio normalization.
      The profile has Protein + Fat + Carbs + Fiber = 1.04 (not 1.0).
      Old code divided by (P+F+C) ignoring Fiber → overallocated macros.
      Fix: Normalize including Fiber, track fiber grams separately.

   2. AAFCO validator now returns corrected grams with debug fields —
      macro_report exposes aafco floors for transparency.

   3. Carbs are recalculated AFTER AAFCO correction so the calorie
      sum remains consistent when protein/fat are bumped up.
─────────────────────────────────────────────────────────────────── */
...
export function calculateMacros({
  ...
  /* ── Ratio normalization (FIX: include Fiber in total) ── */
  const totalRatio =
    (profile.Protein ?? 0) +
    (profile.Fat     ?? 0) +
    (profile.Carbs   ?? 0) +
  ...
  /* ── Initial gram calculation ── */
  const proteinGrams = (calories * proteinRatio) / 4;
  const fatGrams     = (calories * fatRatio)     / 9;
  const carbGrams    = (calories * carbRatio)    / 4;
  const fiberGrams   = (calories * fiberRatio)   / 4; // fiber as carb kcal

  /* ── AAFCO validation — enforces minimums ── */
  ...
  /* ── Recalculate carbs after AAFCO correction ──────────────────
     If AAFCO bumped protein or fat upward, calories are used up.
     Reduce carbs so total calorie sum stays consistent.
  ────────────────────────────────────────────────────────────── */
  const usedCalories =
  ...
  const remainingCaloriesForCarbs = Math.max(0, calories - usedCalories);
  const adjustedCarbGrams = remainingCaloriesForCarbs / 4;
