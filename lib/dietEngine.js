export function generateDietPlan({
  ...
  if (!engineData) {
    throw new Error("engineData missing in diet engine");
  }
  ...
  for (let day = 0; day < 7; day++) {
    ...
    const vegProteinTarget =
      proteinTarget * 0.2;

    let vegQty =
      (vegProteinTarget / Math.max(vegData.protein_g, 0.1)) * 100;
    ...
    let nonVegQty =
      (remainingProtein / Math.max(nonVegData.protein_g, 0.1)) * 100;

    /* ---------- CLINICAL CAPS ---------- */

    const foodName =
      nonVegProtein.toLowerCase();
    ...
    let carbQty =
      (carbTarget / Math.max(carbData.carbs_g, 0.1)) * 100;
